...
// ============================================
// NATURAL TOPIC DATABASE (references rich content)
// ============================================
  ...
  switch (phase) {
    case 'opening':
      if (content.questions?.length) {
        ...
        if (persona.catchphrase && Math.random() < 0.3) q = `${persona.catchphrase}! ${q.toLowerCase()}`;
    ...
    case 'discussion':
      if (wasQuestion && content.answers?.length) {
        ...
        if (persona.catchphrase && Math.random() < 0.4) ans = `${persona.catchphrase} – ${ans.toLowerCase()}`;
        return ans;
      }
      if (messageType === 'question' && content.questions?.length) return getRandomItem(content.questions);
      ...
      if (pool.length) {
        ...
        if (persona.catchphrase && Math.random() < 0.25) msg = `${persona.catchphrase} – ${msg.toLowerCase()}`;
        return msg;
      }
    ...
    default:
      ...
      if (fallbackPool?.length) {
        ...
        if (persona.catchphrase && Math.random() < 0.3) msg = `${persona.catchphrase} – ${msg.toLowerCase()}`;
        return msg;
      }
  ...
  for (const guild of guilds.values()) {
    try {
      ...
      if (success) {
        ...
        // Trim memory
        if (mem.messages.length > CONFIG.maxContextMessages) mem.messages = mem.messages.slice(-CONFIG.maxContextMessages);
        sent++;
      }
...
async function processWelcomeQueue() {
  while (welcomeQueue.length) {
    ...
    if (guild && member) {
      const config = await getGuildConfig(guild.id);
      if (config?.lobby_chatter_enabled) await sendWelcomeMessage(guild, member, config);
    }
  }
}
